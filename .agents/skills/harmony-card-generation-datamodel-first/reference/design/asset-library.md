...
- 表中描述为黑色或黑白实心的 SVG 必须通过静态 `Image.styles.fillColor` 应用当前主题的图标色。无法保证与背景形成清晰对比时，删除该弱图标并重新分配空间给主文字或 CTA。
...
- 静态素材可直接写入 `Image.src`，例如 `"src": "resources/base/media/calendar_fill.svg"`；同时按主题写入静态 `Image.styles.fillColor`。
