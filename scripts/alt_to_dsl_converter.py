#!/usr/bin/env python3
"""Standalone ALT + ASC + TaskSpec -> GenUI DSL converter.

Self-contained copy of the t2d pipeline originally implemented in
scripts/alt_converter.py. This module does not import alt_converter; it only
shares the JSON configuration files under scripts/config/.

Usage: python alt_to_dsl_converter.py --taskspec task.taskSpec.json \
       --alt card.alt.txt --asc card.asc.txt -o card.dsl.jsonl \
       [--theme neutral-light] [--width 200] [--height 120]
"""
...
CONFIG_DIR = Path(_MODULE_FILE).resolve().parent / "config" if _MODULE_FILE else None
THEMES_PATH = CONFIG_DIR / "alt-themes.json" if CONFIG_DIR is not None else None
LAYOUT_PROFILE_PATH = CONFIG_DIR / "alt-layout-profile.json" if CONFIG_DIR is not None else None
TUNING_PATH = CONFIG_DIR / "alt-tuning.json" if CONFIG_DIR is not None else None
...
def configure_logging() -> logging.Logger:
    logger = logging.getLogger("alt_to_dsl_converter")
...
if CONFIG_DIR is not None:
    ...
    # Browser (Pyodide) has no local filesystem; configure_runtime() injects
    # the three JSON configurations after this module finishes loading.
    THEME_CONFIG = None
    LAYOUT_PROFILE = None
    TUNING = None
...
def _derive_tuning_constants() -> None:
    """(Re)compute tuning/theme-derived module constants after config injection."""
    global EPSILON, TEXT_WIDTH_SAFETY, BUTTON_WIDTH_SAFETY, LINE_HEIGHT_PADDING
    global TEXT_MINIMUM_WIDTH_CHARS, LONG_TEXT_UNITS_THRESHOLD, TEXT_UNIT_WEIGHTS
    ...
    AUTO_THEMES = set((THEME_CONFIG or {}).get("themes", {}))


_derive_tuning_constants()
...
def configure_runtime(
    ...
    """Browser entry to inject the three JSON configurations (Pyodide).

    CLI keeps file loading; when called, derived constants are recomputed and
    the injected configuration is validated.
    """
    global THEME_CONFIG, LAYOUT_PROFILE, TUNING
    if themes_json is not None:
        value = json.loads(themes_json)
        if not isinstance(value, dict):
            raise ConversionError("injected themes configuration must be an object")
        THEME_CONFIG = value
    if profile_json is not None:
        value = json.loads(profile_json)
        if not isinstance(value, dict):
            raise ConversionError("injected layout profile must be an object")
        LAYOUT_PROFILE = value
    if tuning_json is not None:
        value = json.loads(tuning_json)
        if not isinstance(value, dict):
            raise ConversionError("injected tuning configuration must be an object")
        TUNING = value
    _derive_tuning_constants()
...
if THEME_CONFIG is not None and LAYOUT_PROFILE is not None and TUNING is not None:
    validate_configuration()
...
def parse_alt_text(text: str) -> AltDocument:
...
def parse_alt(path: Path) -> AltDocument:
    ...
    return parse_alt_text(path.read_text(encoding="utf-8-sig"))
...
def validate_layout(
    document: AltDocument,
    size: str,
    canvas_override: dict[str, float] | None = None,
    ...
    if canvas_override is not None:
        expected = (
            float(canvas_override.get("width", expected[0])),
            float(canvas_override.get("height", expected[1])),
        )
...
def load_task_spec(path: Path) -> dict[str, Any]:
    ...
    return parse_task_spec_text(path.read_text(encoding="utf-8-sig"))
...
def repair_long_auto_text_roles(
    ...
    if canvas_override is not None:
        width = float(canvas_override.get("width", width))
...
def auto_layout_document(
    ...
    if canvas_override is not None:
        width = float(canvas_override.get("width", width))
        height = float(canvas_override.get("height", height))
    data_model = sample_data_model(task_spec["dataModelSchema"])
...
def alt_to_dsl(
    ...
    issues = validate_layout(document, size, canvas_override)
    ...
    if canvas_override is not None:
        width = int(round(float(canvas_override.get("width", width))))
        height = int(round(float(canvas_override.get("height", height))))
...
def build_canvas_override(
    width: float | str | None,
    height: float | str | None,
) -> dict[str, float] | None:
    """Normalize optional width/height overrides; None keeps size-based defaults."""
    override: dict[str, float] = {}
    if width is not None:
        value = float(width)
        if not math.isfinite(value) or value <= 0:
            raise ConversionError(f"invalid card width override {width!r}")
        override["width"] = value
    if height is not None:
        value = float(height)
        if not math.isfinite(value) or value <= 0:
            raise ConversionError(f"invalid card height override {height!r}")
...
def compile_t2d(
    ...
    """Compile one card from text inputs; shared by main() and browser_convert()."""
    task_spec = parse_task_spec_text(task_spec_text)
    document = parse_alt_text(alt_text)
    notes: list[str] = []
    automatic = is_auto_layout(document)
    if automatic:
        if theme is not None:
            if theme not in AUTO_THEMES:
                raise ConversionError(f"unknown ALT theme {theme!r}")
            document.root.attrs["theme"] = theme
        ...
        notes.extend(missing_role_notes)
    elif theme is not None:
        notes.append(f"theme override {theme!r} ignored: ALT is not automatic layout")

    asc = parse_asc_text(asc_text, document)
    ...
    if automatic:
        ...
        role_notes = repair_long_auto_text_roles(
            document, task_spec, asc, canvas_override
        )
        narrow_row_notes = repair_narrow_text_row(
            document, task_spec, asc, canvas_override
        )
        notes.extend(binding_notes)
        notes.extend(role_notes)
        notes.extend(narrow_row_notes)
        ...
        compiled_document, auto_issues = auto_layout_document(
            document, task_spec, asc, canvas_override
        )
        for _ in range(4):
            hard_auto_issues = [
            ...
            if removed_id is None:
                break
            notes.append(f"layout repair: removed low-priority node {removed_id}")
            asc.pop(removed_id, None)
            validate_auto_asc(document, task_spec, asc)
            compiled_document, auto_issues = auto_layout_document(
                document, task_spec, asc, canvas_override
            )
    issues = auto_issues + validate_layout(
        compiled_document, task_spec["size"], canvas_override
    )
...
def browser_convert(
    ...
    """Browser entry point (Pyodide): returns JSON {"dsl", "errors", "warnings"}."""
    try:
        canvas_override = build_canvas_override(width, height)
        result = compile_t2d(
            task_spec_text,
            alt_text,
            asc_text,
        ...
        return compact_json({"dsl": "", "errors": [str(exc)], "warnings": []})
    errors = [
        f"[{issue.node_id}] {issue.message}"
        for issue in result.issues
        if issue.severity == "error"
    ]
    warnings = list(result.notes) + list(result.warnings)
    warnings.extend(
        f"[{issue.node_id}] {issue.message}"
        for issue in result.issues
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Convert a single A2UI Form card from ALT + ASC + TaskSpec to GenUI DSL. "
            "Standalone module; does not import alt_converter.py. Optional --theme "
            "and --width/--height override the card presentation; omitted values "
            "keep the ALT/TaskSpec/config defaults."
        )
    )
    parser.add_argument(
        "--taskspec",
        ...
        type=Path,
        metavar="TASKSPEC",
        help="TaskSpec JSON file path, e.g. task.taskSpec.json.",
    )
    parser.add_argument(
        "--alt",
        required=True,
        type=Path,
        metavar="ALT",
        help="ALT layout file path, e.g. card.alt.txt.",
    )
    parser.add_argument(
        "--asc",
        required=True,
        type=Path,
        metavar="ASC",
        help="ASC semantic companion file path, e.g. card.asc.txt.",
    )
    parser.add_argument(
        "--theme",
        type=str,
        metavar="THEME",
        help=(
            "Override the card theme; must be a declared theme in alt-themes.json. "
            "Omitted: keep the ALT theme or the configured default."
        ),
    )
    parser.add_argument(
        "--width",
        ...
        metavar="VP",
        help=(
            "Override card width in vp (auto layout uses it as the canvas width). "
            "Omitted: keep size default (140 for 2x2, 300 for 2x4)."
        ),
    )
    parser.add_argument(
        "--height",
        ...
        metavar="VP",
        help=(
            "Override card height in vp (auto layout uses it as the canvas height). "
            "Omitted: keep size default (140)."
        ),
    )
    parser.add_argument(
        "-o",
        ...
        type=Path,
        metavar="DSL",
        help="Output GenUI DSL file path, e.g. card.dsl.jsonl.",
    )
    parser.add_argument(
        "--layout-report",
        type=Path,
        metavar="REPORT",
        help="Optional JSON layout report output path; omitted by default.",
    )
...
def main() -> int:
    args = parse_args()
    task_path = args.taskspec.resolve()
    alt_path = args.alt.resolve()
    asc_path = args.asc.resolve()
    output_path = args.output.resolve()
    report_path = args.layout_report.resolve() if args.layout_report is not None else None

    for path in (task_path, alt_path, asc_path):
        if not path.is_file():
            LOGGER.error("input file does not exist: %s", path)
            return 2

    LOGGER.info(
        "START taskspec=%s alt=%s asc=%s output=%s theme=%s width=%s height=%s profile=%s",
        task_path,
        alt_path,
        asc_path,
        output_path,
    ...
    try:
        canvas_override = build_canvas_override(args.width, args.height)
        result = compile_t2d(
            task_path.read_text(encoding="utf-8-sig"),
            alt_path.read_text(encoding="utf-8-sig"),
            asc_path.read_text(encoding="utf-8-sig"),
            theme=args.theme,
            canvas_override=canvas_override,
        )
        for note in result.notes:
            LOGGER.info("repair: %s", note)
        ...
        hard_errors = [issue for issue in result.issues if issue.severity == "error"]
        if result.automatic and hard_errors:
            detail = "; ".join(
                f"{issue.node_id}: {issue.message}" for issue in hard_errors
            )
            raise ConversionError(f"automatic layout is infeasible: {detail}")
        atomic_write(output_path, result.dsl)
        LOGGER.info(
            "DONE alt=%s asc=%s dsl=%s issues=%d",
            alt_path.name,
            asc_path.name,
            output_path.name,
            len(result.issues),
