...
namespace {
constexpr float COLUMN_DEFAULT_SPACE = 8.0F;
}

ColumnTheme::ColumnTheme(const ThemeContext& context) : ThemeBase(context)
{
    InitializeAllProperties();
}

void ColumnTheme::OnConfigChange(const ThemeContext& context)
{
    InitializeAllProperties();
}

void ColumnTheme::InitializeAllProperties()
{
    // DFX: Print theme context information
    LOG_A2UI(LOG_INFO,
        "ColumnTheme::InitializeAllProperties - hasBrandColor=%{public}s, brandColor=0x%{public}x, "
...
A2UIHorizontalAlignment ColumnTheme::ResolveAlignItems(const std::string& align)
{
    if (align == "start") {
...
A2UIFlexAlignment ColumnTheme::ResolveJustifyContent(const std::string& justify)
{
    if (justify == "start") {
...
float ColumnTheme::GetDefaultSpace()
{
    return COLUMN_DEFAULT_SPACE;
}
