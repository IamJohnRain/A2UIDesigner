            ...
            if (parenPos != std::string::npos && expr.back() == ')') {
                ...
                while (pos < argsPart.size()) {
                    ...
                    if (argsPart[valStart] == '\'') {
                        ...
                        pos = endQuote + 1;
                    } else if (argsPart[valStart] == '$' && valStart + 1 < argsPart.size() &&
                               argsPart[valStart + 1] == '{') {
                        ...
                        std::string innerResolved = ResolveTemplate("${" + innerExpr + "}", context);
                        argValue = ParseLooseJsonToken(innerResolved);
                        if (!argValue.IsValid()) {
                            argsValid = false;
                            break;
                        }
                        pos = static_cast<size_t>(innerClose) + 1;
                    } else {
                ...
                if (NativeFunctionRegistry::GetInstance().HasFunction(funcName)) {
                    if (!argsValid || !argsObject.IsValid()) {
                        resolved.clear();
                        i = static_cast<size_t>(closePos) + 1;
                        result << resolved;
                        continue;
                    }
                    ResolvedValue funcResult =
                        NativeFunctionRegistry::GetInstance().Execute(funcName, argsObject, context);
                    if (!funcResult.success) {
                        resolved.clear();
                    } else {
                        resolved = JsonValueToTemplateOutput(funcResult.value);
                    }
                }
            } else {
                ...
                if (dataModel != nullptr) {
                    ...
                    if (nodeOpt.has_value()) {
                        resolved = JsonValueToTemplateOutput(nodeOpt.value());
                    }
                }
            }
