...
The ALT tab also supports BYOK LLM generation (user query → TaskSpec → ALT/ASC) through OpenAI-compatible `chat/completions` endpoints. Settings (BaseURL/model/API Key) live in `localStorage`; the API Key is encrypted with WebCrypto (PBKDF2-SHA-256 310000 iterations → AES-GCM) and decrypted only with the user master password, which is never persisted. Prompt templates are plain Markdown under `scripts/alt-prompts/` (template + `reference/` appendix files) and are fetched at generation time, so editing them takes effect without code changes.
